...
class Sniffer():
    def __init__(self):
		...
		"""
        self.Gateway=[]
        self.pingstat=threading.Thread(target=self.ping_network)
        try:
            open('sfile.db')
            flush_tables()
        except:
    ...
    def ping_network(self):
        for ip in self.Gateway:
            ip='.'.join(ip.split('.')[:-1])
            for i in range(255):
                os.system('ping '+ip+str(i))
    ...
    def map_ip_to_mac(self, ip, mac):
        if mac != '00:00:00:00:00:00' and ip != '0.0.0.0':
            if ip not in self.iptomacmap.keys():
                self.iptomacmap[ip]=[mac]
                add_ip_mac(ip, mac)
                if ip.split('.')[-1]=='1':
                    self.Gateway.append(ip)
            else:
                if mac not in self.iptomacmap[ip]:
                    self.iptomacmap[ip].append(mac)
                    add_ip_mac(ip, mac)
        if self.verbose_imt:        
            self.print_mapiptomac()
        for ip in self.iptomacmap:
        	if ip.split('.')[-1]!='1' and len(self.iptomacmap[ip])>1:
        		self.block_ip(ip,self.iptomacmap[ip])
    ...
    def sniff_packet(self):
        ...
        if protocol == 8 and (self.verbose_udp or self.verbose_tcp or self.verbose_icmp):
            ...
            if protocol2 == 1 and self.verbose_icmp:
                string = self.icmp_protocol(packet, header_length + eth_header_length * 4)
                if string:
                    print string
            elif protocol2 == 6 and self.verbose_tcp:
                string =  self.tcp_protocol(packet, header_length + eth_header_length * 4)
                if string:
                    print string
            elif protocol2 == 17 and self.verbose_udp:
                string =  self.udp_protocol(packet, header_length + eth_header_length * 4)
                if string:
                    print string
            else:
                pass
            ...
            if not self.gui:
                string =  self.arp_protocol(packet)
                if string:
                    print string
            else:
